...
int main(void) {
    int request;
    socketif_t socket1, socket2;
    char hostAddress[MAXADDRESSLEN];
    int port;
    int res = 0;

    // Inizializzazione dell’interfaccia socket sulla porta 20000
    socket1 = createUDPInterface(35000);
    socket2 = createUDPInterface(35000);
    ...
    while(request != 0){
        ...
        UDPReceive(socket1, &request, sizeof(request), hostAddress, &port);
        UDPReceive(socket2, &request, sizeof(request), hostAddress, &port);
